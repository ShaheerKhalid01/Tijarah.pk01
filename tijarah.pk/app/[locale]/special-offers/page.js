...
import { specialOffers } from '@/app/lib/product-data';

// ✅ OPTIMIZED: Memoized product card component
const ProductCard = memo(({ product, locale, t }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        <button
          onClick={handleClick}
          className="mt-3 w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors active:scale-95"
          type="button"
        >
          {t('viewDeal')}
        </button>
...
const ProductsGrid = memo(({ currentProducts, locale, t }) => {
  return (
...
export default function SpecialOffersPage() {
  ...
  const categories = useMemo(() => {
    const counts = {};
    specialOffers.forEach(p => {
      counts[p.category] = (counts[p.category] || 0) + 1;
    });
    return Object.entries(counts).map(([id, count]) => ({ id, name: id.charAt(0).toUpperCase() + id.slice(1), count }));
  }, []);
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              ...
              <select
                name="sort"
                className="block w-full sm:w-auto rounded-md border-gray-300 pl-3 pr-10 py-2 text-base focus:border-red-500 focus:outline-none focus:ring-red-500"
                value={sortBy}
            ...
            {currentProducts.length > 0 ? (
              <>
                <ProductsGrid currentProducts={currentProducts} locale={locale} t={t} />
